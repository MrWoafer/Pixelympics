...
public class Sprint200RaceController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        recordText.text = PlayerPrefs.GetFloat("200m Record", 20f).ToString("n2");
        Debug.Log("Current record is: " + PlayerPrefs.GetFloat("200m Record", 20f).ToString());
    ...
    public void Finish(string playerName, float time, bool eligibleForRecord)
    {
        ...
        if (time < PlayerPrefs.GetFloat("200m PB " + playerName, 1000f) && eligibleForRecord)
        {
            Debug.Log(playerName + " got a new PB!");
            PlayerPrefs.SetFloat("200m PB " + playerName, time);
        }
        ...
        if (!winner)
        {
            ...
            if (time < PlayerPrefs.GetFloat("200m Record", 20f) && eligibleForRecord)
            {
                Debug.Log(playerName + " got a new record!");
                PlayerPrefs.SetFloat("200m Record", time);
                recordText.text = "WR";
            }
        }
